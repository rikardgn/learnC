//Updated version of ovvpn program that launches passmanager only if you give it a third arguemtn thats the path of the passmanager.
...
int main(int argc, char** argv)
{
    ...
    int maxIndVpn=0, indVpnSel, i, k, siteFileExists, timeStart, timeDiff, timeCur;
    ...
    if(argc==3){
        ...
        //wait 60s approx before lanuching ovpn
        timeStart=time(NULL);
        while(true){
            timeCur=time(NULL);
            timeDiff=timeCur-timeStart;
            if(timeDiff>63){
                printf("Exiting!");
                break;
            }
